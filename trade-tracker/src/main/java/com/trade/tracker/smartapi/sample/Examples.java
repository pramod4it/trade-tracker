...
import com.trade.tracker.smartapi.models.EstimateChargesParams;
import com.trade.tracker.smartapi.models.Gtt;
import com.trade.tracker.smartapi.models.GttParams;
import com.trade.tracker.smartapi.models.MarginParams;
...
import com.trade.tracker.smartapi.orderupdate.OrderUpdateListner;
import com.trade.tracker.smartapi.orderupdate.OrderUpdateWebsocket;
import com.trade.tracker.smartapi.smartTicker.SmartWSOnConnect;
import com.trade.tracker.smartapi.smartTicker.SmartWSOnDisconnect;
import com.trade.tracker.smartapi.smartTicker.SmartWSOnError;
import com.trade.tracker.smartapi.smartTicker.SmartWSOnTicks;
import com.trade.tracker.smartapi.smartTicker.SmartWebsocket;
import com.trade.tracker.smartapi.smartstream.models.SmartStreamError;
import com.trade.tracker.smartapi.ticker.OnConnect;
import com.trade.tracker.smartapi.ticker.OnTicks;
import com.trade.tracker.smartapi.ticker.SmartAPITicker;
import com.trade.tracker.smartapi.utils.Constants;
...
public class Examples {
	...
	public void placeOrder(SmartConnect smartConnect) throws SmartAPIException, IOException {
		...
		orderParams.setVariety(Constants.VARIETY_STOPLOSS); // Set the variety (e.g., stop loss order)
        orderParams.setQuantity(1); // Order quantity
        orderParams.setSymbolToken("1660"); // Symbol token for the instrument
        orderParams.setExchange(Constants.EXCHANGE_NSE); // Exchange (e.g., NSE)
        orderParams.setOrderType(Constants.ORDER_TYPE_STOPLOSS_LIMIT); // Order type (stoploss limit)
        orderParams.setTradingSymbol("ITC-EQ"); // Trading symbol for the stock
        orderParams.setProductType(Constants.PRODUCT_INTRADAY); // Product type (e.g., Intraday)
        orderParams.setDuration(Constants.DURATION_DAY); // Duration for the order (Day)
        orderParams.setTransactionType(Constants.TRANSACTION_TYPE_BUY); // Transaction type (Buy or Sell)
        orderParams.setPrice(122.2); // Price for the limit order
        orderParams.setTriggerPrice("209"); // Trigger price for the stop loss (as String or double)
